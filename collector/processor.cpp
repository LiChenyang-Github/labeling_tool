...
void Processor::splitVideo(QString videoPath, QString dir, QString baseName)
{
    ...
    int counter = 0;	//计数器用于十帧采一帧
    qDebug() << totalFrameNumber << endl;
    for (int i = 0; i < totalFrameNumber; i++)
    {
        bool writeSuccessfully;
        Mat frame;
        ...
        if ((i % 10) == 0)
        {
            ...
            QString writeImgPath = dir + "/" + baseName + "_color_" + QString::number(counter) + ".png";
            writeSuccessfully = imwrite(writeImgPath.toStdString().data(), frame);
            if (writeSuccessfully == false)
            {
                QMessageBox::information(NULL, "Path", "Existing Chinese path!");
                break;
            }
            imshow("show", frame);
            waitKey(10);
...
Mat Processor::prepareRecordedImagePairEight(vector<float> v)
{
    ...
    tlx_l = v[24]; tly_l = v[25];
    brx_l = v[26]; bry_l = v[27];
    btx_l = v[28]; bty_l = v[29];
    bjx_l = v[30]; bjy_l = v[31];
    iftx_l = v[32]; ifty_l = v[33];
    ifjx_l = v[34]; ifjy_l = v[35];
    mftx_l = v[36]; mfty_l = v[37];
    mfjx_l = v[38]; mfjy_l = v[39];
...
Mat Processor::prepareRecordedImagePairTen(vector<float> v)
{
    ...
    mftx_l = v[36]; mfty_l = v[37];
    mfjx_l = v[38]; mfjy_l = v[39];
    rftx_l = v[40]; rfty_l = v[41];
    rfjx_l = v[42]; rfjy_l = v[43];
    tftx_l = v[44]; tfty_l = v[45];
    tfjx_l = v[46]; tfjy_l = v[47];
