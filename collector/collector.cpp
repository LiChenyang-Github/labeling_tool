...
void collector::on_startButt_clicked()
{
    ...
    {
        QMessageBox::information(NULL, tr("Empty Path"), tr("Please Open a Directory!"));
        return;
    }
    ...
    //shortcut of next butt
    QShortcut *key_prePoint = new QShortcut(QKeySequence(Qt::Key_W), this);//创建一个快捷键"W"键
    connect(key_prePoint, SIGNAL(activated()), this, SLOT(on_lastPoint_clicked()));//连接到指定槽函数
...
void collector::on_lastPoint_clicked()
{
    proc.clickedTimes--;
    if (proc.clickedTimes == -1)
    {
        proc.clickedTimes = 0;
        QMessageBox::information(NULL, tr(""), tr("Arrive Minimal Point!"));
        return;
    }
}
...
{
    proc.imgSign = false;	//mark as a low quality img
...
void collector::recordDataIntoDb(QString videoType)
{
    ...
    if (proc.checkPoints(videoType))
    {
        ...
        db.execInsert(index, currentImageName, params, proc.imgSign, videoType);
    }
    else
    {
...
void collector::displayMat(Mat image)
{
    ...
    ui->imageLabel->setPixmap(QPixmap::fromImage(img));	//将img放入到imageLabel中
    ui->imageLabel->resize(ui->imageLabel->pixmap()->size());	//调整imageLabel的大小使得其大小和img一样大
}
...
void collector::displayData()
{
    ...
    //###right hand
    QString tlxy = tr("左上:(") + QString::number(proc.tlx) + "," + QString::number(proc.tly) + ")";	//tr用于显示中文
    ui->tlLabel->setText(tlxy);

    QString btxy = tr("拇指指尖:(") + QString::number(proc.btx) + "," + QString::number(proc.bty) + ")";
    ui->btLabel->setText(btxy);

    QString bjxy = tr("拇指关节:(") + QString::number(proc.bjx) + "," + QString::number(proc.bjy) + ")";
    ui->bjLabel->setText(bjxy);

    QString brxy = tr("右下:(") + QString::number(proc.brx) + "," + QString::number(proc.bry) + ")";
    ui->brLabel->setText(brxy);

    QString iftxy = tr("食指指尖:(") + QString::number(proc.iftx) + "," + QString::number(proc.ifty) + ")";
    ui->iftLabel->setText(iftxy);

    QString ifjxy = tr("食指关节:(") + QString::number(proc.ifjx) + "," + QString::number(proc.ifjy) + ")";
    ui->ifjLabel->setText(ifjxy);

    QString mftxy = tr("中指指尖:(") + QString::number(proc.mftx) + "," + QString::number(proc.mfty) + ")";
    ui->mftLabel->setText(mftxy);

    QString mfjxy = tr("中指关节:(") + QString::number(proc.mfjx) + "," + QString::number(proc.mfjy) + ")";
    ui->mfjLabel->setText(mfjxy);

    QString rftxy = tr("无名指指尖:(") + QString::number(proc.rftx) + "," + QString::number(proc.rfty) + ")";
    ui->rftLabel->setText(rftxy);

    QString rfjxy = tr("无名指关节:(") + QString::number(proc.rfjx) + "," + QString::number(proc.rfjy) + ")";
    ui->rfjLabel->setText(rfjxy);

    QString tftxy = tr("尾指指尖:(") + QString::number(proc.tftx) + "," + QString::number(proc.tfty) + ")";
    ui->tftLabel->setText(tftxy);

    QString tfjxy = tr("尾指关节:(") + QString::number(proc.tfjx) + "," + QString::number(proc.tfjy) + ")";
    ui->tfjLabel->setText(tfjxy);


    //left hand
    QString tlxy_l = tr("左上:(") + QString::number(proc.tlx_l) + "," + QString::number(proc.tly_l) + ")";
    ui->tlLabel_l->setText(tlxy_l);

    QString brxy_l = tr("右下:(") + QString::number(proc.brx_l) + "," + QString::number(proc.bry_l) + ")";
    ui->brLabel_l->setText(brxy_l);

    QString btxy_l = tr("拇指指尖:(") + QString::number(proc.btx_l) + "," + QString::number(proc.bty_l) + ")";
    ui->btLabel_l->setText(btxy_l);

    QString bjxy_l = tr("拇指关节:(") + QString::number(proc.bjx_l) + "," + QString::number(proc.bjy_l) + ")";
    ui->bjLabel_l->setText(bjxy_l);

    QString iftxy_l = tr("食指指尖:(") + QString::number(proc.iftx_l) + "," + QString::number(proc.ifty_l) + ")";
    ui->iftLabel_l->setText(iftxy_l);

    QString ifjxy_l = tr("食指关节:(") + QString::number(proc.ifjx_l) + "," + QString::number(proc.ifjy_l) + ")";
    ui->ifjLabel_l->setText(ifjxy_l);

    QString mftxy_l = tr("中指指尖:(") + QString::number(proc.mftx_l) + "," + QString::number(proc.mfty_l) + ")";
    ui->mftLabel_l->setText(mftxy_l);

    QString mfjxy_l = tr("中指关节:(") + QString::number(proc.mfjx_l) + "," + QString::number(proc.mfjy_l) + ")";
    ui->mfjLabel_l->setText(mfjxy_l);

    QString rftxy_l = tr("无名指指尖:(") + QString::number(proc.rftx_l) + "," + QString::number(proc.rfty_l) + ")";
    ui->rftLabel_l->setText(rftxy_l);

    QString rfjxy_l = tr("无名指关节:(") + QString::number(proc.rfjx_l) + "," + QString::number(proc.rfjy_l) + ")";
    ui->rfjLabel_l->setText(rfjxy_l);

    QString tftxy_l = tr("尾指指尖:(") + QString::number(proc.tftx_l) + "," + QString::number(proc.tfty_l) + ")";
    ui->tftLabel_l->setText(tftxy_l);

    QString tfjxy_l = tr("尾指关节:(") + QString::number(proc.tfjx_l) + "," + QString::number(proc.tfjy_l) + ")";
